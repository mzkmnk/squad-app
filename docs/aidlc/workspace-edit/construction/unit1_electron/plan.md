...
- [ ] `electron/types/ipc-error-code.ts` に `DUPLICATE_ENTRY` エラーコードを追加

### 2. IPC チャネル定数・リクエスト型追加

- [ ] `electron/ipc/ipc-channels.ts` に `WORKSPACE_ADD_ENTRY` / `WORKSPACE_REMOVE_ENTRY` チャネル定数を追加
- [ ] `WorkspaceAddEntryRequest` / `WorkspaceRemoveEntryRequest` インターフェースを追加

### 3. テスト作成（workspace:add-entry）

- [ ] テスト: 正常系 — 単一エントリ追加（fetch → addWorktree → updateWorkspace → generate）
- [ ] テスト: 正常系 — 複数エントリ一括追加
- [ ] テスト: 異常系 — Workspace が見つからない（NOT_FOUND）
- [ ] テスト: 異常系 — Repository が見つからない（NOT_FOUND）
- [ ] テスト: 異常系 — 同一リポジトリの重複エントリ（DUPLICATE_ENTRY）
- [ ] テスト: 異常系 — Worktree 作成失敗時のロールバック
- [ ] テスト: 異常系 — fetch 失敗（GIT_OPERATION_FAILED）
...
- [ ] テスト: 正常系 — 複数エントリ一括削除
- [ ] テスト: 異常系 — Workspace が見つからない（NOT_FOUND）
- [ ] テスト: 異常系 — 指定 repositoryId がエントリに存在しない（VALIDATION_ERROR）
- [ ] テスト: 正常系 — Worktree 削除失敗でもストア更新は続行（ベストエフォート）
- [ ] テスト: 正常系 — Repository が見つからなくても削除続行

### 5. ハンドラー実装（workspace:add-entry）

- [ ] `electron/ipc/ipc-handlers.ts` に `workspace:add-entry` ハンドラーを実装
...
- [ ] `electron/ipc/ipc-handlers.ts` に `workspace:remove-entry` ハンドラーを実装
  - Workspace 存在確認 → repositoryId 検証 → Worktree 削除（ベストエフォート）→ updateWorkspace → generate

### 7. preload / ElectronAPI 更新

- [ ] `electron/preload.ts` に `addWorkspaceEntry` / `removeWorkspaceEntry` メソッドを追加
- [ ] `electron/types/electron-api.ts` の `ElectronAPI` インターフェースに型定義を追加
