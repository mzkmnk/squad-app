...
- [ ] `getRemoteBranches()` 呼び出しの前に `gitService.fetch(repo.name)` を追加
- [ ] fetch は独立した try/catch で囲み、失敗時（ネットワークエラー含む）もキャッシュ済みのブランチ一覧を返す（ベストエフォート）。ハンドラ全体の try/catch に巻き込まれないようにする

### 2. `WORKSPACE_CREATE` ハンドラに fetch を追加（`electron/ipc/ipc-handlers.ts`）

- [ ] worktree 作成ループ（ステップ3「各エントリに対して Worktree を作成」）の各イテレーション冒頭で `gitService.fetch(resolved.repo.name)` を追加
- [ ] 同一リポジトリの重複 fetch を避けるため、fetch 済みリポジトリ名を `Set<string>` で管理する
- [ ] fetch 失敗時はエラーを返す（`WORKSPACE_ADD_ENTRY` と同じ動作: fetch 失敗 → ロールバック → エラー返却）

### 3. `BackgroundFetchService` の作成（`electron/git/background-fetch-service.ts`）

- [ ] `BackgroundFetchService` クラスを新規作成
...
- [ ] テスト作成（`electron/git/background-fetch-service.spec.ts`）

### 4. メインプロセスへの組み込み（`electron/main.ts`）

- [ ] `BackgroundFetchService` インスタンスをモジュールスコープの変数として保持する（`initializeServices()` のスコープ外からアクセスするため）
- [ ] `initializeServices()` 内で `BackgroundFetchService` を生成・`start()` を呼び出す
- [ ] `app.on('before-quit')` で `stop()` を呼び出す

### 5. テスト

- [ ] 既存テストの修正: `REPO_BRANCHES` の既存テストに `gitService.fetch` のモック設定を追加（fetch 追加により既存テストが失敗するため）
- [ ] 既存テストの修正: `WORKSPACE_CREATE` の既存テストに `gitService.fetch` のモック設定を追加
- [ ] `REPO_BRANCHES` ハンドラのテスト: fetch が呼ばれた後にブランチ一覧が返ることを確認
- [ ] `REPO_BRANCHES` ハンドラのテスト: fetch 失敗時もブランチ一覧が返ることを確認
- [ ] `WORKSPACE_CREATE` ハンドラのテスト: 各リポジトリに対して fetch が呼ばれることを確認
- [ ] `WORKSPACE_CREATE` ハンドラのテスト: 同一リポジトリが複数エントリにある場合、fetch は1回のみ
- [ ] `BackgroundFetchService` のテスト: start/stop/fetchAll の動作確認
